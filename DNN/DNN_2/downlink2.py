...
def cre_v(Hreal,Himag,vrf,ld,seta=0.1):        #创建混合预编码矩阵V=VRF*VD
    ...
    N_real1,N_imag1=mydata_2.nk(g_M)
    N_real2,N_imag2=mydata_2.nk(g_M)
    N_real3,N_imag3=mydata_2.nk(g_M)
    N_real4,N_imag4=mydata_2.nk(g_M)                                             #四个用户的噪声矩阵
    ...
    for i in range(ld-1):
        ...
        N_real2,N_imag2=mydata_2.nk(g_M)
        N_real3,N_imag3=mydata_2.nk(g_M)
        N_real4,N_imag4=mydata_2.nk(g_M)
        N_real=np.c_[N_real1,N_real2,N_real3,N_real4]
        N_imag=np.c_[N_imag1,N_imag2,N_imag3,N_imag4]
...
user_hk_real,user_hk_imag=mydata_2.cre_hk_real(6,64)
# print(user_hk_real.T,-user_hk_imag.T)

yk=mydata_2.cre_yk(64,999,user_hk_real,user_hk_imag,1)
...
# out=layers.Dense(256,activation=tf.nn.relu)(out)
...
for i in range(100000):
    if i%100==0:
        print('循环了',i,'次')
    vrf1=model(mydata_2.yk1).numpy().T           #生成K个用户的vrf
    # print('vrf1=',vrf1,vrf1.shape)

    # user_hk_real2,user_hk_imag2=mydata_2.cre_hk_real(6,g_M)
    # yk2=mydata_2.cre_yk(g_M,0,user_hk_real2,user_hk_imag2,1)
    # yk2=tf.convert_to_tensor(yk2,dtype=tf.float32)    
    vrf2=model(mydata_2.yk2).numpy().T
    # print('vrf2=',vrf2,vrf2.shape)

    # user_hk_real3,user_hk_imag3=mydata_2.cre_hk_real(6,g_M)
    # yk3=mydata_2.cre_yk(g_M,0,user_hk_real3,user_hk_imag3,1)
    # yk3=tf.convert_to_tensor(yk3,dtype=tf.float32)    
    vrf3=model(mydata_2.yk3).numpy().T
    # print('vrf3=',vrf3,vrf3.shape)

    # user_hk_real4,user_hk_imag4=mydata_2.cre_hk_real(6,g_M)
    # yk4=mydata_2.cre_yk(g_M,0,user_hk_real4,user_hk_imag4,1)
    # yk4=tf.convert_to_tensor(yk4,dtype=tf.float32)    
    vrf4=model(mydata_2.yk4).numpy().T
    # print('vrf4=',vrf4,vrf4.shape)

    vrf=np.c_[vrf1,vrf2,vrf3,vrf4]        #堆叠成VRF的相位
    # print('vrf=',vrf,vrf.shape)
    hreal=np.c_[mydata_2.user_hk_real1,mydata_2.user_hk_real2,mydata_2.user_hk_real3,mydata_2.user_hk_real4]     #信道矩阵H
    himag=np.c_[mydata_2.user_hk_imag1,mydata_2.user_hk_imag2,mydata_2.user_hk_imag3,mydata_2.user_hk_imag4]
    # print('hreal=',hreal,hreal.shape)
    # print('himag=',himag,himag.shape)


    Vreal1,Vimag1=cre_v(hreal,himag,vrf,3)         #生成ld=3的时候的V，计算用户总速率
    rate11=rate_puser(mydata_2.user_hk_real1,mydata_2.user_hk_imag1,Vreal1[:,0],Vimag1[:,0],Vreal1[:,1],Vimag1[:,1],Vreal1[:,2],Vimag1[:,2],Vreal1[:,3],Vimag1[:,3])
    rate12=rate_puser(mydata_2.user_hk_real2,mydata_2.user_hk_imag2,Vreal1[:,1],Vimag1[:,1],Vreal1[:,0],Vimag1[:,0],Vreal1[:,2],Vimag1[:,2],Vreal1[:,3],Vimag1[:,3])
    rate13=rate_puser(mydata_2.user_hk_real3,mydata_2.user_hk_imag3,Vreal1[:,2],Vimag1[:,2],Vreal1[:,1],Vimag1[:,1],Vreal1[:,0],Vimag1[:,0],Vreal1[:,3],Vimag1[:,3])
    rate14=rate_puser(mydata_2.user_hk_real4,mydata_2.user_hk_imag4,Vreal1[:,3],Vimag1[:,3],Vreal1[:,0],Vimag1[:,0],Vreal1[:,2],Vimag1[:,2],Vreal1[:,1],Vimag1[:,1])
    ...
    rate21=rate_puser(mydata_2.user_hk_real1,mydata_2.user_hk_imag1,Vreal2[:,0],Vimag2[:,0],Vreal2[:,1],Vimag2[:,1],Vreal2[:,2],Vimag2[:,2],Vreal2[:,3],Vimag2[:,3])
    rate22=rate_puser(mydata_2.user_hk_real2,mydata_2.user_hk_imag2,Vreal2[:,1],Vimag2[:,1],Vreal2[:,0],Vimag2[:,0],Vreal2[:,2],Vimag2[:,2],Vreal2[:,3],Vimag2[:,3])
    rate23=rate_puser(mydata_2.user_hk_real3,mydata_2.user_hk_imag3,Vreal2[:,2],Vimag2[:,2],Vreal2[:,1],Vimag2[:,1],Vreal2[:,0],Vimag2[:,0],Vreal2[:,3],Vimag2[:,3])
    rate24=rate_puser(mydata_2.user_hk_real4,mydata_2.user_hk_imag4,Vreal2[:,3],Vimag2[:,3],Vreal2[:,0],Vimag2[:,0],Vreal2[:,2],Vimag2[:,2],Vreal2[:,1],Vimag2[:,1])
    ...
    rate31=rate_puser(mydata_2.user_hk_real1,mydata_2.user_hk_imag1,Vreal3[:,0],Vimag3[:,0],Vreal3[:,1],Vimag3[:,1],Vreal3[:,2],Vimag3[:,2],Vreal3[:,3],Vimag3[:,3])
    rate32=rate_puser(mydata_2.user_hk_real2,mydata_2.user_hk_imag2,Vreal3[:,1],Vimag3[:,1],Vreal3[:,0],Vimag3[:,0],Vreal3[:,2],Vimag3[:,2],Vreal3[:,3],Vimag3[:,3])
    rate33=rate_puser(mydata_2.user_hk_real3,mydata_2.user_hk_imag3,Vreal3[:,2],Vimag3[:,2],Vreal3[:,1],Vimag3[:,1],Vreal3[:,0],Vimag3[:,0],Vreal3[:,3],Vimag3[:,3])
    rate34=rate_puser(mydata_2.user_hk_real4,mydata_2.user_hk_imag4,Vreal3[:,3],Vimag3[:,3],Vreal3[:,0],Vimag3[:,0],Vreal3[:,2],Vimag3[:,2],Vreal3[:,1],Vimag3[:,1])
